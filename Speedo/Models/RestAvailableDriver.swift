...
struct ApiRestAvailableDriver : Codable {
    
    let message : String?
    let result : [ResRestAvailableDriver]?
    let status : String?
    ...
    init(from decoder: Decoder) throws {
        ...
        result = try values.decodeIfPresent([ResRestAvailableDriver].self, forKey: .result)
        status = try values.decodeIfPresent(String.self, forKey: .status)
    }
    
}

struct ResRestAvailableDriver : Codable {
